...
public class StrBuilderExample {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 - String 클래스 단점으로 인한 메모리 과부하 밑 속도가 느려지는 현상을
		 개선하기 위해서 StringBuilder가 새롭게 추가되었습니다
		 
		 - StringBuilder는 기존 객체를 계속 재활요하면서 실제 객체 내부의 값을
		 변경하게 됩니다.
		 
		 - StringBuilder, StringBuffer는 메서드가 동일합니다.
		 StringBuilder -> 단일 스레드
		 StringBuffer -> 다중 스레드에서 문자열을 공유해야 할 떄.
		...
		StringBuilder sb = new StringBuilder("hello"); // 생성자에 매개값에 넣고 싶은 값 넣기
		System.out.println(sb);
		System.out.println("sb의 주소값: " + sb.hashCode());
		
		// 문자열을 맨 끝에 추가는 메서드 append(문자열)
		sb.append(" world ");
		System.out.println(sb);
		
		// 문자열을 특정 인덱스에 삽입하는 메서드 insert(인데스, 삽입할 문자열)
		sb.insert(6, "my ");
		System.out.println(sb);
