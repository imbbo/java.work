...
public class RussianRouletteSol {
	...
	public static void main(String[] args) {
		...
		while(true) {						
			...
			if(bulletPos[realBulletPos]) { // 총알 격발
				...
				// 지금 죽은 사람 기준으로 뒤에 있는 값을 한칸씩 떙기는 작업.
//				int[] x = new int[players.length-1];
				for(int i=startIdx; i<players.length-1; i++) {
					players[i] = players[i+1]; // 총알이 소모 되었으니 false로 변경
