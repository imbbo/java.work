			...
			if(menu == 1) {
				...
				while(true) {
					...
					if(!flag) { // flag == false => 중복 발생x
						...
						empNum = sc.next(); // 기존에 변수에 새로운 값 넣기
					}
				}
				...
				if(count == 0) {
					System.out.println("등록된 사원 정보가 업습니다.");					
				} else {
				...
				if(i == count) {
					System.out.println("조회하신 사원벙보는 존재하지 않습니다.");
				}
				...
				//					if(i == num) {
				//						System.out.print("1. 나이변경 ㅣ2. 부서변경 ㅣ3. 취소\n> ");
				//						int cho = sc.nextInt();
				//						
				...
				for(int i=0; i<count; i++) {
					if(empNum.equals(userNums[i])) {
						...
						System.out.println("[1. 나이 변경 ㅣ 2. 부서 변경 ㅣ 3. 취소]");
						System.out.print("> ");
						int sel = sc.nextInt();
