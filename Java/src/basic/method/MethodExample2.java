...
1. 매게변수는 메서드를 호출할 때, 메서드 실행에 필요한 값들을
메서드 내부로 전달하는 매개체 역할을 합니다.

2. 메서드 호출부에서 어떤 매개값을 전달하느냐에 따라
메서드의 실행 결과는 달라질 수 있습니다.

3. 매게값을 몇 개 받아서 사용할지는 메서드를 선언할 떄 선언부에서
()안에 미리 개수와 타입을 지정합니다.

4. 매개 변수의 이름을 지을 때는 호출자가 선언부로 돌아와서
매개값을 확인하지 않게, 어떠한 값을 담는 변수인지
그 뜻을 알기 쉽게 지어주면 서로 좋습니다.

5. 매개변수를 하나도 선언하지 않을 수도 있습니다. 이 떄는 메서드의 선언부에 
...
public class MethodExample2 {
	...
	static int calcRangeTotal(int x, int y) { // 변수의 이름은 의미가 있는 이름으로 해주면 좋다
		int total = 0;
//		if(x > y) {
//			int temp = x;
//			x = y;
//			y = temp;  만약에 x값이 더 크고 y값이 더 작은 상황이면 if문으로 스위치 해줄수있다
